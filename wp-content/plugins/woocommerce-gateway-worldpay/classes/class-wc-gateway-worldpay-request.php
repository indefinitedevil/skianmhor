...
class WC_Gateway_WorldPay_Request {
	...
	public static function get_worldpay_args( $order ) {
		...
		$billing_postcode 		= $pre_wc_30 ? $order->billing_postcode : $order->get_billing_postcode();
		...
		$shipping_first_name 	= $pre_wc_30 ? $order->shipping_first_name : $order->get_shipping_first_name();
		$shipping_last_name 	= $pre_wc_30 ? $order->shipping_last_name : $order->get_shipping_last_name();
		$shipping_address_1 	= $pre_wc_30 ? $order->shipping_address_1 : $order->get_shipping_address_1();
		$shipping_address_2 	= $pre_wc_30 ? $order->shipping_address_2 : $order->get_shipping_address_2();
		$shipping_city 			= $pre_wc_30 ? $order->shipping_city : $order->get_shipping_city();
		$shipping_state 		= $pre_wc_30 ? $order->shipping_state : $order->get_shipping_state();
		$shipping_postcode 		= $pre_wc_30 ? $order->shipping_postcode : $order->get_shipping_postcode();
	...
	protected static function get_worldpay_order_num( $order ) {
		...
		if ( get_post_meta( $order_id,'_order_number',TRUE ) && is_plugin_active( 'woocommerce-sequential-order-numbers/woocommerce-sequential-order-numbers.php' ) ) {

			return get_post_meta( $order_id,'_order_number',TRUE );

		} elseif ( get_post_meta( $order_id,'_order_number_formatted',TRUE ) && is_plugin_active( 'woocommerce-sequential-order-numbers-pro/woocommerce-sequential-order-numbers.php' ) ) {
		
			return get_post_meta( $order_id,'_order_number_formatted',TRUE );
